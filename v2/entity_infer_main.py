...
def get_dataloader(conf):
    ...
    train_discriminator_data = None #loaders.DiscriminatorDataset(conf, feature_folder, train_caption_path)
    train_discriminator_loader = None #loaders.DataLoader(train_discriminator_data, batch_size=conf.discriminator_batch_size, shuffle=True, collate_fn=lambda batch: loaders.discriminator_collate_fn(batch, conf))
    
    val_supervised_data = loaders.EntitySupervisedDataset(conf, feature_folder, val_caption_path)
    val_supervised_loader = loaders.DataLoader(val_supervised_data, batch_size=conf.supervised_batch_size, shuffle=False, collate_fn=lambda batch: loaders.entity_supervised_collate_fn(batch, conf))
    val_discriminator_data = None #loaders.DiscriminatorDataset(conf, feature_folder, val_caption_path)
    val_discriminator_loader = None #loaders.DataLoader(val_discriminator_data, batch_size=conf.discriminator_batch_size, shuffle=True, collate_fn=lambda batch: loaders.discriminator_collate_fn(batch, conf))
...
def main():
    ...
    conf = entity_infer_config.myConfig()
    parse_opt(conf)
