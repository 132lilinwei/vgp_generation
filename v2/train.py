...
#             print(target_sent, target_sent_len)
            pred_word_probs, sampled_sent, sampled_sent_len = g_model(img_feature, target_sent, target_sent_len)
...
#     d_model.to(config.device)
    ...
    for e in range(num_epoch):
        ...
        for idx, data in enumerate(train_dataloader):
            ...
            img_feature, target_sent, target_sent_len, wrong_sent, wrong_sent_len = data["image"], data["caption"], data["length"], data["wrong_caption"], data["wrong_length"]
            img_feature, target_sent, target_sent_len, wrong_sent, wrong_sent_len = utils.convert_to_device(config.device, img_feature, target_sent, target_sent_len, wrong_sent, wrong_sent_len)
            ...
            print(decode_sentence(target_sent, i2w))
#             if e == 2:
#                 print('pred_scores:', pred_scores)
            ...
            ge_pred_scores = d_model(img_feature, sampled_sent, sampled_sent_len)
...
def eval_d(config, g_model, d_model, eval_dataloader):
    ...
    with torch.no_grad():
        ...
        for idx, data in enumerate(eval_dataloader):
            ...
            # Here is ground truth loss
            img_feature, target_sent, target_sent_len, wrong_sent, wrong_sent_len = data["image"], data["caption"], data["length"], data["wrong_caption"], data["wrong_length"]
            img_feature, target_sent, target_sent_len, wrong_sent, wrong_sent_len = utils.convert_to_device(config.device, img_feature,
                                                                                target_sent, target_sent_len, wrong_sent, wrong_sent_len)
            batch_size = img_feature.size(0)
            target_label = torch.ones(batch_size).to(config.device)
            pred_scores = d_model(img_feature, target_sent, target_sent_len)
#             print('pred_scores:', pred_scores)
...
#     torch.save(g_model.state_dict(), P_join(config.checkpoint_output, "g_after_supervised.pth"))
    ...
    train_d(config, g_model, d_model, train_discriminator_loader, eval_discriminator_loader, config.d_train_epoch, i2w)
