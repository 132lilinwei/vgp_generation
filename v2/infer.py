...
def entity_infer(config, g_model, dataloader, i2w):
    ...
    g_model.eval()
    for idx, data in enumerate(dataloader):
        img_feature, target_entity, target_entity_len = data["image"], data["caption"], data["length"]
        img_feature, target_entity, target_entity_len = utils.convert_to_device(config.device, img_feature,
                                                                            target_entity, target_entity_len)
