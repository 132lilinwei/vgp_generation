...
def train_g(config, g_model, train_dataloader, eval_dataloader, num_epoch, caller="supervised"):
    ...
    for e in range(num_epoch):
        ...
        for idx, data in enumerate(train_dataloader):
            ...
            entity_feature, neighbor_feature_list, img_feature, coarse_target_sent, coarse_target_sent_len, fine_target_sent, fine_target_sent_len, exist_mask = data['entity'], data[
                'neighbor'], data["image"], data["caption"], data["length"], data['fine_caption'], data['fine_length'], data['fine_exist']
            entity_feature, neighbor_feature_list, img_feature, coarse_target_sent, coarse_target_sent_len, fine_target_sent, fine_target_sent_len, exist_mask = utils.convert_to_device(
                config.device, entity_feature, neighbor_feature_list, img_feature, coarse_target_sent, coarse_target_sent_len, fine_target_sent, fine_target_sent_len, exist_mask)

            # pred_word_probs: [batch, seq_len-1, vocab], sampled_sent: [batch, seq_len], sampled_sent_len: [batch]
            l1_pred_words_probs, l1_sampled_sent, l1_sampled_sent_len, l2_pred_words_probs, l2_sampled_sent, l2_sampled_sent_len = g_model(entity_feature, neighbor_feature_list,
                                                                      img_feature, coarse_target_sent, coarse_target_sent_len, fine_target_sent, fine_target_sent_len)
            ...
            l1_flat_pred_word_probs = l1_pred_words_probs.reshape(-1, l1_pred_words_probs.size(2))
            l1_flat_sampled_sent = l1_sampled_sent[:, 1:].reshape(-1)  # ignore START
            l1_loss = config.g_crit(l1_flat_pred_word_probs, l1_flat_sampled_sent)  # [batch_size * (seq_len - 1)]
            l1_mask = (l1_flat_sampled_sent != config.pad_idx).type(torch.FloatTensor).to(config.device)
            loss = torch.sum(l1_loss * l1_mask) / torch.sum(l1_mask)  # compute mean

            l2_flat_pred_word_probs = l2_pred_words_probs.reshape(-1, l2_pred_words_probs.size(2))
            l2_flat_sampled_sent = l2_sampled_sent[:, 1:].reshape(-1)   # ignore START
            l2_loss = config.g_crit(l2_flat_pred_word_probs, l2_flat_sampled_sent)  # [B * (seq_len - 1)]
            l2_mask = exist_mask.expand((l2_pred_words_probs.size(0), l2_pred_words_probs.size(1))).reshape(-1).type(torch.FloatTensor).to(config.device)             # mask only coarse case
            l2_mask *= (l2_flat_sampled_sent != config.pad_idx).type(torch.FloatTensor).to(config.device) # mask pad
            loss += torch.sum(l2_loss * l2_mask) / torch.sum(l2_mask)
        ...
        if (e + 1) % 10 == 0 and eval_dataloader is not None:
            ...
            torch.save(g_model.state_dict(),
                       P_join(config.checkpoint_output, "g_" + caller + "_epoch" + str(e) + ".pth"))
