...
class EntityBiAttnSupervisedDataset(Dataset):
    ...
    def __init__(self, config, image_feature_folder, entity_feature_folder, entity_caption_path, transform=None):
        ...
        for file_item in dataset:
            ...
            for entity_item in file_item['entities']:
                entity_id = entity_item['phrase_id']
                if len(entity_item['entity_descs']) == 1:
                    desc_item = entity_item['entity_descs'][0]
                    ...
                    self.fine_desc_ids.append(None)
                    self.captions.append(desc)
                    self.fine_captions.append(None)
                    self.neighbors.append(neighbor_entities)
                    
                elif len(entity_item['entity_descs']) > 1:
                    # look for coarse item
                    coarse_item = None
                    for item in entity_item['entity_descs']:
                        if item['tf_idf_level'] == 0:
                            coarse_item = item
                    entity_desc_id = coarse_item['entity_desc_id']
                    desc = coarse_item['indexed']
                    
                    # here we add START and END
                    desc = [int(self.config.start_idx)] + desc + [int(self.config.end_idx)]
                            
                    # iterate over fine item
                    for item in entity_item['entity_descs']:
                        if item['entity_desc_id'] == entity_desc_id:
                            continue
                        fine_desc_id = item['entity_desc_id']
                        fine_desc = item['indexed']
                        # here we add START and END
                        fine_desc = [int(self.config.start_idx)] + fine_desc + [int(self.config.end_idx)]
                        
                        self.caption_names.append(filename)
                        self.entity_ids.append(entity_id)
                        self.desc_ids.append(entity_desc_id)
                        self.fine_desc_ids.append(fine_desc_id)
                        self.captions.append(desc)
                        self.fine_captions.append(fine_desc)
                        self.neighbors.append(neighbor_entities)                   
    ...
    def __getitem__(self, idx):
        ...
        fine_desc_id = self.fine_desc_ids[idx]
        caption = self.captions[idx]
        fine_caption = self.fine_captions[idx] if fine_desc_id is not None else caption
        neighbors = self.neighbors[idx]
        entity_feature = np.load(os.path.join(self.entity_feature_folder, str(entity_id)+".npy"))
        
        fine_exist = torch.ByteTensor(1)
        fine_exist[0] = fine_desc_id is not None
        ...
        sample = {'entity_feature': torch.tensor(entity_feature), "caption": torch.tensor(caption), \
                    ...
                    "fine_desc_id": fine_desc_id, "fine_caption": torch.tensor(fine_caption), "fine_exist": fine_exist}
        
        return sample
    
def entity_bi_attn_supervised_collate_fn(batch, config):
    """
    ...
    fine_caption_list = [item['fine_caption'] for item in batch]
    fine_caption_length = torch.tensor([len(item) for item in fine_caption_list]).view(-1, 1)
    fine_final_captions = pad_sequence(fine_caption_list, batch_first=True, padding_value=config.pad_idx)
    
    fine_exist = [item['fine_exist'] for item in batch]
    final_fine_exist = torch.stack(fine_exist)
    ...
    fine_desc_id_list = [item['fine_desc_id'] for item in batch]
    ...
    final_batch['fine_caption'] = fine_final_captions 
    final_batch['fine_length'] = fine_caption_length
    final_batch['caption_name'] = caption_name_list
    final_batch['entity_id'] = entity_id_list
    final_batch['desc_id'] = desc_id_list
    final_batch['fine_desc_id'] = fine_desc_id_list
    final_batch['fine_exist'] = final_fine_exist
    return final_batch
