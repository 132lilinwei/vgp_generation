...
class discriminator(nn.Module):
    def __init__(self, config, word_embed=None):
        ...
        self.relu = nn.ReLU()
        self.img_to_hidden = nn.Linear(config.image_hidden, config.d_hidden_size)
    ...
    def forward(self, img_feature, sentence, sen_seq_len):
        ...
        # we simply use the last hidden unit in last layer of the END token
        pack_out, (h, c) = self.lstm(pack_sent)
        h = self.tanh(h.squeeze(0)) #[1,128, hidden_size] ->
