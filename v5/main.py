...
def get_dataloader(conf):
    ...
    train_supervised_data = loaders.EntityBiAttnSupervisedDataset(conf, image_feature_folder, entity_feature_folder, train_caption_path)
    train_supervised_loader = loaders.DataLoader(train_supervised_data, batch_size=conf.supervised_batch_size, num_workers=8, shuffle=True, collate_fn=lambda batch: loaders.entity_bi_attn_supervised_collate_fn(batch, conf))
    ...
    val_supervised_loader = loaders.DataLoader(val_supervised_data, batch_size=conf.supervised_batch_size, num_workers=8, shuffle=True, collate_fn=lambda batch: loaders.entity_bi_attn_supervised_collate_fn(batch, conf))
