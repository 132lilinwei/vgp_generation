...
def write_entity_to_json_file(entity_list, target_list, caption_name_list, entityid_list, desc_id_list, filepath):
    ...
    for entity, target, caption, entityid, desc in zip(entity_list, target_list, caption_name_list, entityid_list, desc_id_list):
        ...
        target_id = [item['desc_id'] for item in temp_dict['target']]
        if desc not in target_id:
...
def write_corse_to_fine_to_json_file(entity_list, fine_entity_list, target_list, fine_target_list, caption_name_list, entityid_list, desc_id_list, fine_desc_id_list, filepath):
    """
    Each entityid maps to one dict {'caption_name': , 'entityid':, 'pred': [], 'target': [{'entity':, 'desc_id': }]}
    """
    final_dict = {}
    for entity, fine_entity, target, fine_target, caption, entityid, desc_id, fine_desc_id in zip(entity_list, fine_entity_list, target_list, fine_target_list, caption_name_list, entityid_list, desc_id_list, fine_desc_id_list):
        temp_dict = final_dict.get(entityid, {'caption_name': caption,
                                             'entityid': entityid,
                                             'pred': [], 'fine_pred': [],
                                             'target': [], 'fine_target':[]})
        if entity not in temp_dict['pred']:
            temp_dict['pred'].append(entity)
        if fine_entity not in temp_dict['fine_pred']:
            temp_dict['fine_pred'].append(fine_entity)

        target_id = [item['desc_id'] for item in temp_dict['target']]
        if desc_id not in target_id:
            temp_dict['target'].append({'entity': target, 'desc_id': desc_id})
        
        if fine_desc_id is not None:
            target_id = [item['desc_id'] for item in temp_dict['fine_target']]
            if fine_desc_id not in target_id:
                temp_dict['fine_target'].append({'entity': fine_target, 'desc_id': fine_desc_id})
