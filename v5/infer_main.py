...
def get_dataloader(conf):
    ...
    val_supervised_data = loaders.EntityBiAttnSupervisedDataset(conf, image_feature_folder, entity_feature_folder, val_caption_path)
    val_supervised_loader = loaders.DataLoader(val_supervised_data, batch_size=conf.supervised_batch_size, num_workers=8, shuffle=False, collate_fn=lambda batch: loaders.entity_bi_attn_supervised_collate_fn(batch, conf))
    
    val_discriminator_data = None
    val_discriminator_loader = None
...
def main():
    ...
    # print config
    utils.print_config(conf)
    
    # generate sentence
#     entity_list, target_list, caption_name_list, entityid_list, desc_id_list, l2_entity_list, l2_target_list, l2_caption_name_list, l2_entityid_list, l2_desc_id_list = infer.entity_infer(conf, g_model, eval_supervised_loader, i2w)
#     utils.write_to_files(entity_list, conf.generated_path)
#     utils.write_to_files(l2_entity_list, 'l2_'+conf.generated_path)
#     utils.write_to_files(target_list, 'target_'+conf.generated_path)
#     utils.write_to_files(l2_target_list, 'l2_target_'+conf.generated_path)
...
#     l2_final_dict = utils.write_entity_to_json_file(l2_entity_list, l2_target_list, l2_caption_name_list, l2_entityid_list, l2_desc_id_list, 'l2_'+conf.generated_path[:-4]+'.json')
#     metric_eval.eval_txt(conf.generated_path, 'target_'+conf.generated_path)
    
#     print("l1_acc")
#     metric_eval.eval_entity(final_dict)
#     print("l2_acc")
#     metric_eval.eval_entity(l2_final_dict)
    
    print("diversity")
    entity_list, target_list, caption_name_list, entityid_list, desc_id_list, l2_entity_list, l2_target_list, _, _, l2_desc_id_list = infer.entity_infer(conf, g_model, eval_supervised_loader, i2w)
    final_dict = utils.write_corse_to_fine_to_json_file(entity_list, l2_entity_list, target_list,l2_target_list,  caption_name_list, entityid_list, desc_id_list, l2_desc_id_list, conf.generated_path[:-4]+'_diversity.json')
    metric_eval.eval_diversity_entity(final_dict)
